...
/// Connects to the backend socket.io server at BackendConfig.BaseUrl.
/// </summary>
public class ChatManager : MonoBehaviour
{
    ...
    {
        string url  = $"{BackendConfig.BaseUrl}/api/chat/send";
    ...
    {
        string url = $"{BackendConfig.BaseUrl}/api/chat/history?channel={channel.ToString().ToLower()}&limit=50";
    ...
    {
        string url = $"{BackendConfig.BaseUrl}/api/chat/dm/thread/{friendId}?limit=100";
        using var req = UnityWebRequest.Get(url);
        req.timeout = 10;
        req.SetRequestHeader("Authorization", $"Bearer {BackendConfig.AuthToken}");
        yield return req.SendWebRequest();
