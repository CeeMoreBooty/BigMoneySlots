...
public class SlotMachine : MonoBehaviour
{
    ...
    private IEnumerator SpinCoroutine()
    {
        ...
        // ── Progressive jackpot evaluation (contributes bet to pools) ──
        if (ProgressiveJackpot.Instance != null)
        {
            var (winTier, jackpotPrize) = ProgressiveJackpot.Instance.EvaluateSpin(_currentBet);
            if (winTier.HasValue && jackpotPrize > 0)
            {
                jackpotPrize = LoyaltySystem.Instance?.ApplyTierBonus(jackpotPrize) ?? jackpotPrize;
                if (PlayerEconomy.Instance != null) PlayerEconomy.Instance.AddCoins(jackpotPrize);
                else GameManager.Instance?.userData?.AddCoins(jackpotPrize);
                LoyaltySystem.Instance?.RegisterSpin();
                _isSpinning = false;
                _reelResult = GenerateResult();
                OnSpinFinished?.Invoke(_reelResult, jackpotPrize, WinLevel.EpicWin);
                OnSpinComplete?.Invoke(jackpotPrize, true);
                GameManager.Instance?.OnSpinComplete(jackpotPrize, WinLevel.EpicWin);
                yield break;
            }
        }
        ...
        // Apply RTP from active game config
        float rtp = SlotGameLoader.Instance?.ActiveGame?.baseRTP ?? 0.70f;
        if (payout > 0 && UnityEngine.Random.value > rtp)
            payout = 0;
        ...
        // Recalculate win level after RTP adjustment
        if (payout <= 0) winLevel = WinLevel.None;
